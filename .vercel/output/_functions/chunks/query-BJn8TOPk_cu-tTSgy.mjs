import { _ as __exportAll, e as encodeCursor, g as getI18nConfig, h as isI18nEnabled, m as getFallbackChain } from './types-ByV5sgsv_DJ3e8Y6Y.mjs';
...
import { C as CURSOR_RAW_VALUES } from './loader-Chm5h7Gr_VuvI1mhf.mjs';
...
async function getEmDashCollectionUncached(type, filter) {
	const { getLiveCollection } = await import('./_astro_content_BUtf6gbh.mjs');
...
async function getEmDashEntry(type, id, options) {
	const { getLiveEntry } = await import('./_astro_content_BUtf6gbh.mjs');
	const ctx = getRequestContext();
...
async function hydrateEntryBylines(type, entries) {
	...
	try {
		const { getBylinesForEntries } = await import('./bylines-BYHWU3T7_BCHhh3TY.mjs').then((n) => n.t);
...
async function hydrateEntryTerms(type, entries) {
	if (entries.length === 0) return;
	try {
		const { getAllTermsForEntries } = await import('./taxonomies-WamPVA2x_leJ5kPza.mjs').then((n) => n.u);
...
async function getTranslations(type, id) {
	try {
		const db = (await import('./loader-Chm5h7Gr_VuvI1mhf.mjs').then(n => n.l).then((n) => n.i)).getDb;
		const dbInstance = await db();
		const { ContentRepository } = await import('./content-C0ooIs-f_Bwo8eX_E.mjs').then((n) => n.n);
...
async function resolveEmDashPath(path) {
	if (!cachedUrlPatterns) {
		const { getDb } = await import('./loader-Chm5h7Gr_VuvI1mhf.mjs').then(n => n.l).then((n) => n.i);
